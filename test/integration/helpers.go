...
func createTestNamespace(name string, client kubecli.KubevirtClient) {
	...
	Eventually(func() k8s.NamespacePhase {
		ns, err := client.CoreV1().Namespaces().Get(context.Background(), name, k8smeta.GetOptions{})
		Expect(err).ToNot(HaveOccurred())
		return ns.Status.Phase
	}).WithTimeout(2 * time.Minute).WithPolling(5 * time.Second).Should(Equal(k8s.NamespaceActive))
}

func deleteTestNamespace(name string, client kubecli.KubevirtClient) {
...
func deleteVMIAndWait(vmi *v1.VirtualMachineInstance, client kubecli.KubevirtClient) {
	...
	Expect(libwait.WaitForVirtualMachineToDisappearWithTimeout(vmi, 2*time.Minute)).To(Succeed())
}
...
func withoutLoopbackInterface(guestNetDevNames []string) []string {
	...
	Expect(loIfaceIndex).NotTo(Equal(-1), "loopback interface is not present in the guest")
...
func checkGuestIfaces(
	...
	Expect(len(netIfaceNames)).To(Equal(len(macAddresses)))

	for i, guestVdpaIface := range netIfaceNames {
		guestMacAddr := guestNetDevMacAddress(vmi, guestVdpaIface)
		Expect(guestMacAddr).To(Equal(macAddresses[i]))
	}
